...
// ... (保留之前所有的常量定义：NEEDLE_COUNT, ORNAMENT_COUNT 等，不需要变) ...
...
// ... (保留所有 createHammeredBumpMap, createGiftWrapBumpMap 等辅助函数，完全不变) ...
...
export const LuxuryTree: React.FC<LuxuryTreeProps> = ({ treeState, extraRotationVelocity, userTextureUrls, isPhotoFocused, zoomFactor }) => {
  ...
  useEffect(() => {
    ...
    // 关键：允许加载跨域图片（OSS），否则 WebGL 会变黑或报错
  ...
  // ... (保留余下的 hooks: sphereBumpMap, boxBumpMap, Materials 定义等，完全不变) ...
